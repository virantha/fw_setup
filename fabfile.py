...
def first_setup():
    ...
    local("mkvirtualenv fw_setup")

    # pip install packages
    with prefix('workon fw_setup'):
        local("pip install pytest")

def build_windows_dist():
    if os.name == 'nt':
        # Call the pyinstaller
        local("python ../pyinstaller/pyinstaller.py fw_setup_windows.spec --onefile")
...
def run_tests():
    ...
    with lcd(test_dir):
        ...
        t = local("py.test --cov-config .coveragerc --cov=fw_setup --cov-report=term --cov-report=html", capture=False)
...
def push_docs():
    ...
    """
    githubpages = "/Users/virantha/dev/githubdocs/fw_setup"
    ...
    with lcd("docs"):
        print("Running sphinx in docs/ and building to ~/dev/githubpages/fw_setup")
        local("make clean")
        local("make html")
